"""
Enhanced main orchestrator for PDF preprocessing pipeline with hierarchical parsing
"""
import os
import sys
import logging
...
from extract_hierarchical import extract_pdf_content_hierarchical, test_hierarchical_extraction
...
class HierarchicalPDFPreprocessor:
    """Enhanced PDF preprocessor with hierarchical parsing capabilities"""
    ...
    def process_all_documents(self) -> Dict[str, bool]:
        ...
        logger.info("Starting enhanced PDF preprocessing pipeline")
        logger.info(f"Hierarchical parsing: {'ENABLED' if self.use_hierarchical else 'DISABLED'}")
    ...
    def process_single_document(self, pdf_filename: str, json_filename: str) -> bool:
        """
        Process a single PDF document through the enhanced pipeline
        ...
        try:
            ...
            # Step 2: Extract content with hierarchical parsing
            logger.info(f"Step 1: Extracting content with {'hierarchical' if self.use_hierarchical else 'legacy'} parser...")
            doc_id = json_filename.replace('.json', '')
            
            # Use PDF title if available, otherwise use filename
            title = pdf_filename.replace('.pdf', '')
            
            structured_doc = extract_pdf_content_hierarchical(
                str(pdf_path), doc_id, title, use_hierarchical=self.use_hierarchical
            )
            
            if not structured_doc.get('sections'):
                logger.error("No sections created during extraction/structuring")
                return False
            
            logger.info(f"Created {len(structured_doc['sections'])} sections")
            
            # Step 3: Enrich with metadata (using existing enrichment)
            logger.info("Step 2: Enriching with domain metadata...")
            enriched_doc = enrich_document_content(structured_doc)
            
            # Step 4: Validate the result
            logger.info("Step 3: Validating processed document...")
            ...
            # Step 6: Print processing statistics
    ...
    def test_single_document(self, pdf_filename: str, output_filename: str = None) -> bool:
        """
        Test hierarchical extraction on a single document
        
        Args:
            pdf_filename: Name of the PDF file to test
            output_filename: Optional output filename for test results
        ...
        try:
            ...
            logger.info(f"Testing hierarchical extraction on {pdf_filename}")
            test_hierarchical_extraction(str(pdf_path), str(output_path) if output_path else None)
            
            return True
            
        except Exception as e:
            logger.error(f"Test failed for {pdf_filename}: {e}")
            logger.exception("Full traceback:")
            return False
    ...
    def _print_document_stats(self, document: Dict[str, Any], pdf_filename: str) -> None:
        ...
        for section in sections:
            ...
            # Count hierarchical information if available
            hierarchy = section.get('hierarchy', {})
            if hierarchy:
                hierarchy_type = hierarchy.get('type', 'unknown')
                hierarchy_counts[hierarchy_type] = hierarchy_counts.get(hierarchy_type, 0) + 1
        ...
        if hierarchy_counts:
            logger.info(f"   Hierarchical types: {dict(hierarchy_counts)}")
        
        # Print sample hierarchical paths if available
        hierarchical_sections = [s for s in sections if s.get('hierarchy', {}).get('parent_path')]
        if hierarchical_sections:
            logger.info("   Sample hierarchical paths:")
            for section in hierarchical_sections[:3]:
                hierarchy = section['hierarchy']
                logger.info(f"     {hierarchy.get('parent_path', 'N/A')} > {hierarchy.get('identifier', 'N/A')}")
...
def main():
    """Main entry point for the enhanced preprocessing pipeline"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Enhanced PDF preprocessing with hierarchical parsing")
    parser.add_argument("--disable-hierarchical", action="store_true", 
                       help="Disable hierarchical parsing (use legacy method)")
    parser.add_argument("--test", type=str, metavar="PDF_FILE",
                       help="Test hierarchical extraction on a single PDF file")
    parser.add_argument("--test-output", type=str, metavar="OUTPUT_FILE",
                       help="Output file for test results (optional)")
    
    args = parser.parse_args()
    
    try:
        use_hierarchical = not args.disable_hierarchical
        processor = HierarchicalPDFPreprocessor(use_hierarchical=use_hierarchical)
        
        if args.test:
            # Test mode
            success = processor.test_single_document(args.test, args.test_output)
